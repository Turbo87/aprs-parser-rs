...
pub fn parse_latitude(s: &str) -> Result<f32, APRSError> {
    ...
    let deg = s[0..2].parse::<u32>().map_err(|_| APRSError::InvalidLatitude(s.to_owned()))? as f32;
    let min = s[2..4].parse::<u32>().map_err(|_| APRSError::InvalidLatitude(s.to_owned()))? as f32;
    let min_frac = s[5..7].parse::<u32>().map_err(|_| APRSError::InvalidLatitude(s.to_owned()))? as f32;
...
pub fn parse_longitude(s: &str) -> Result<f32, APRSError> {
    ...
    let deg = s[0..3].parse::<u32>().map_err(|_| APRSError::InvalidLongitude(s.to_owned()))? as f32;
    let min = s[3..5].parse::<u32>().map_err(|_| APRSError::InvalidLongitude(s.to_owned()))? as f32;
    let min_frac = s[6..8].parse::<u32>().map_err(|_| APRSError::InvalidLongitude(s.to_owned()))? as f32;
...
mod tests {
    ...
    fn test_latitude() {
        ...
        assert_eq!(parse_latitude("4903.50W"), Err(APRSError::InvalidLatitude("4903.50W".to_owned())));
        assert_eq!(parse_latitude("4903.50E"), Err(APRSError::InvalidLatitude("4903.50E".to_owned())));
    ...
    fn test_longitude() {
        ...
        assert_eq!(parse_longitude("04903.50N"), Err(APRSError::InvalidLongitude("04903.50N".to_owned())));
        assert_eq!(parse_longitude("04903.50S"), Err(APRSError::InvalidLongitude("04903.50S".to_owned())));
